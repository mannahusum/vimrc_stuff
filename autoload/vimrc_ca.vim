...
" function s:get_start_editor_services() abort
"   if isdirectory(expand(g:powershell_editor_services))
"     let l:start_editor_services = expand(g:powershell_editor_services)
"         \ . '/Start-EditorServices.ps1'
"     set hidden

"     if executable(l:powershell)

"       let s:pslogdir = expand(l:statedir . '/pses.log')
...
"   return l:start_editor_services
" endfunction
...
"   if isdirectory(expand(g:powershell_editor_services))
"     let l:start_editor_services = s:get_start_editor_services

"     if filereadable(l:start_editor_services)
...
"     " Rename - rn => rename
"     noremap <buffer> <leader>rn <Plug>(coc-rename)
...
"     nnoremap <buffer> <C-k><C-r> <Plug>(coc-references)

"     setlocal formatexpr=LanguageClient_textDocument_rangeFormatting()
"     vnoremap <buffer> = <Plug>(coc-format-selected)
"     nnoremap <buffer> <C-e><C-d> <Plug>(coc-format)
"     vnoremap <buffer> <silent> <F8> <Plug>(coc-codeaction-selected)
"   endif
...
" function! vimrc_ca#add_coc() abort
" endfunction
